...
function M.lint(linter_cmd, linter_handler)
	...
	utils.lint(temp_file, linter_cmd, function(output, run_err)
		pcall(os.remove, temp_file)
		running_checks_by_buf[buf] = nil
		local diagnostics = linter_handler(output, run_err)

		-- Create a unique namespace for norminette diagnostics
		local namespace = vim.api.nvim_create_namespace(linter_cmd)
